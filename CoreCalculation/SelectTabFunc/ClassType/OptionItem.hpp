...
#include "CoreCalculation/SelectTabFunc/StringCodec.hpp"


namespace CoreCalculation::Base
{
    ...
    public: auto operator<=>(const OptionItem& other) const {
            return _index <=> other._index;
        }
    public: bool operator==(const OptionItem& other) const {
            return _index == other._index;
        }
    ...
    inline QDebug &operator<<(QDebug &debug, const QVector<Group>& groups) {
        debug << "[";
        if (groups.empty()) return debug << "]";
        debug << groups[0].group();
        for (int i = 1; i < groups.size(); ++i) {
            debug << "," << groups[i].group();
        }
        return debug << "]";
    }
