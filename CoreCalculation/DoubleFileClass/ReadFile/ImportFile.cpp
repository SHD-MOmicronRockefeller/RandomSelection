...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    QRegularExpression keyValueRegex(R"xx((\w+)=([\w\(\)\s,]+))xx");
    QRegularExpression joinGroupRegex(R"xx(joinGroup\(([^)]+)\))xx");
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    for (int i = 0; i < lines.size(); ++i) {
        ...
        // 2. option+名称段
        sortedParts.append(QString("option %1").arg(namePart));
        // 3. weight段
...
QString RsolProcessor::generateMachineCode(const QString& formattedContent) {
    ...
    QRegularExpression optionRegex(R"xx(option\s+([^\s]+))xx");          // 提取option后的名称
    ...
    for (const QString& line : lines) {
        ...
        // 3. 提取option名称，生成__OPTION__指令
        QString optionName = "";
        QRegularExpressionMatch optionMatch = optionRegex.match(trimLine);
        if (optionMatch.hasMatch()) {
            optionName = optionMatch.captured(1);
            machineCode.append(QString("__OPTION__ %1 %2").arg(indexVal).arg(optionName));
        }
        ...
        // ========== 新增核心逻辑：所有option强制关联基础组__BASELIST__ ==========
...
// 核心函数：读取文件 + 全流程处理
QStringList RsolProcessor::getCode(const QString& filePath) {
    // 读取文件内容
    QFile file(filePath);
    ...
    QStringList machineCodeList = machineCode.split(QRegularExpression("\\n"), Qt::SkipEmptyParts);
